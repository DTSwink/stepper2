param(
    [string]$Prefix = ("timed_full_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
)
...
try {
    ...
    $Stage1 = "${Prefix}_stage1_k1_fast_overfit"
    $Stage2 = "${Prefix}_stage2_ar_8_16_32"
    $Stage3 = "${Prefix}_stage3_k32_polish"
    $Stage4 = "${Prefix}_stage4_k32_final"

    Run-Stage "stage1_k1_fast_overfit" $Stage1 @(
        "--folder-path", ".\data\npz_final",
        "--run-name", $Stage1,
        "--device", "cuda",
        "--batch-size", "64",
    ...
    Run-Stage "stage2_ar_8_16_32" $Stage2 @(
        "--folder-path", ".\data\npz_final",
        "--run-name", $Stage2,
    ...
    Run-Stage "stage3_k32_polish_5e-6" $Stage3 @(
        "--folder-path", ".\data\npz_final",
        "--run-name", $Stage3,
    ...
    Run-Stage "stage4_k32_final_2e-6" $Stage4 @(
        "--folder-path", ".\data\npz_final",
        "--run-name", $Stage4,
