...
## 2026-05-20 - Canonical GT-Diff Recipe Runner Is Now Hard To Mislaunch

- Reason:
  we had one failed reproduction because the flexible orchestration exposed too
  many flags. In particular, using `agent_batch_clips=1` instead of the packed
  per-agent path changed the controller shelf. The successful recipe should not
  be relaunched by manually remembering a long flag list.
- Code change:
  `training/train_locomotion_self_adversarial.py` is now a narrow canonical
  runner. It exposes only:
  dataset folders, starting controller checkpoint, starting prior checkpoint,
  run name, cycle count, device/eval device, key clip names, and `--no-evaluate`.
  The fragile recipe choices are hard-coded:
  - AE cycles: reconstruction-only, fresh negatives only, no fake buffer,
    `compatibility_real_weight=0`, `compatibility_fake_weight=0`,
    `fake_margin=0.04`, `real_weight=1.25`, `fake_weight=1.0`,
    `fake_starts_per_clip=10`, `fake_rollout_steps=32`,
    `hard_negative_mode=low_energy_high_gtdiff`,
    `hard_negative_keep_fraction=0.7`.
  - Controller cycles: pure AE prior, `compatibility_score_weight=0`,
    no contact/footslide physics losses, no `torch.compile`,
    packed per-agent rollout with `agent_batch_clips=0`,
    K schedule `2,4,8,16,32`, mixed cohorts `5,15,20,30,40`.
- Exact reproduction from the saved GT-diff cycle-2 state:
  command to rerun from a fresh context:
  ```powershell
...
- Clean reproduction run:
  `training/runs/20260520_214701_clean_exact_repro_from_cycle2`.
  It reproduced the saved values exactly:
  - Cycle 1, equivalent to saved reconstruction-only cycle 3:
    controller `best_val=0.010423826985061169`.
  - Cycle 2, equivalent to saved reconstruction-only cycle 4:
    controller `best_val=0.01020110584795475`.
  - Cycle 3, equivalent to saved reconstruction-only cycle 5:
    controller `best_val=0.011211628094315529`.
- Physical monitor equality:
  diagnostic output:
  `training/runs/diagnostics/clean_exact_repro_from_cycle2_20260520`.
  Saved cycle 5 and clean reproduction cycle 5 matched exactly on the mini-set:
  mean support p95 `0.13528687157668173`, mean source-contact excess p95
  `0.16815690422663465`, max source-contact excess p95
  `0.33907386660575867`.

## 2026-05-20 - Canonical Recipe From Old K32 Does Not Reproduce The Same Shelf Yet

- Purpose:
  after proving the cleaned runner exactly reproduces the known cycle-2-start
  recipe, test whether the same canonical recipe also converges cleanly when
  restarted from the older K32 baseline and the original one-frame prior.
- Command:
  ```powershell
  .\.tools\python310\python.exe training\train_locomotion_self_adversarial.py `
    --periodic-folder-path training\runs\mini_datasets\idle_walkF_circle_stand45\periodic `
    --nonperiodic-folder-path training\runs\mini_datasets\idle_walkF_circle_stand45\nonperiodic `
    --start-controller-checkpoint training\runs\20260516_044547_hybrid_canonbasis_from_bestk32_constantk32_w010_resumeopt\checkpoints\checkpoint_best_k32.pt `
    --start-prior-checkpoint training\runs\20260517_161820_denoise_rootlook1_lat32_n0p05_e360\checkpoints\checkpoint_best.pt `
    --run-name clean_from_oldk32 `
    --cycles 5 `
    --device cuda `
    --eval-device cpu `
    --no-evaluate
  ```
- Run:
  `training/runs/20260520_221233_clean_from_oldk32`.
- Controller AE-loss checkpoints by cycle:
  - cycle 1: `best_val=0.010220642201602459`.
  - cycle 2: `best_val=0.012548768892884254`.
  - cycle 3: `best_val=0.01189617719501257`.
  - cycle 4: `best_val=0.0140564925968647`.
  - cycle 5: `best_val=0.012971676886081696`.
- Physical monitor comparison on the mini-set:
  diagnostics at `training/runs/diagnostics/clean_from_oldk32_20260520`.
  - Saved canonical cycle 5 / exact repro:
    mean support p95 `0.135287`, mean source-contact excess p95 `0.168157`,
    max source-contact excess p95 `0.339074`.
  - From-old-K32 cycle 1:
    mean support p95 `0.246490`, mean source-contact excess p95 `0.413391`,
    max source-contact excess p95 `1.161697`.
  - From-old-K32 cycle 3:
    mean support p95 `0.166809`, mean source-contact excess p95 `0.240746`,
    max source-contact excess p95 `0.528019`.
  - From-old-K32 cycle 5:
    mean support p95 `0.131833`, mean source-contact excess p95 `0.191911`,
    max source-contact excess p95 `0.441061`.
- Interpretation:
  the cleaned runner itself is correct and exact when started from the known
  cycle-2 state. However, starting from the old K32 baseline does not yet
  reproduce the same physical shelf after five cycles. It clearly improves
  versus its own cycle 1, especially on circles, but remains worse than the
  saved canonical cycle-5 monitor. Do not conflate the exact reproduction proof
  with this from-old-K32 convergence check.

## 2026-05-20 - Replay Fakes Ablation On The Bad Cycle-4 To Cycle-5 Transition

- Question:
  cycle 4 to cycle 5 from the old-K32 run regressed on the GT-diff selector
  metric. Test whether training the cycle-5 AE on older fake rollouts as well
  as fresh cycle-4 fakes prevents forgetting.
- Setup:
  start from
  `training/runs/20260520_221233_clean_from_oldk32/controller_cycle_04/checkpoints/checkpoint_best_k32.pt`
  and
  `training/runs/20260520_221233_clean_from_oldk32/ae_cycle_04/checkpoints/checkpoint_best.pt`.
  Build a fake buffer from hard negatives generated by old-K32 controller
  cycles 1, 2, and 3, selected by the same `low_energy_high_gtdiff` rule under
  AE cycle 4. Then train the alternate cycle-5 AE with those replay fakes plus
  fresh cycle-4 fakes.
- Resulting replay controller:
  `training/runs/20260520_231521_replay_ablation_from_oldk32_cycle4/controller_replay_cycle5_from_c4/checkpoints/checkpoint_best_k32.pt`.
- GT-diff selector metric:
  - cycle 4 fresh baseline:
    mean `0.336598`, p95 `0.525606`, max `0.717605`.
  - normal fresh-only cycle 5:
    mean `0.357363`, p95 `0.622987`, max `0.760449`.
  - replay old1-3 + fresh cycle4:
    mean `0.358362`, p95 `0.615655`, max `0.728524`.
- Physical monitor:
  - normal fresh-only cycle 5:
    mean support p95 `0.131833`, mean source-excess p95 `0.191911`,
    max source-excess p95 `0.441061`.
  - replay old1-3 + fresh cycle4:
    mean support p95 `0.143507`, mean source-excess p95 `0.227044`,
    max source-excess p95 `0.514735`.
- AE energy audit on hard negatives:
  replay AE did remember old fakes better:
  old cycle-2 fake margin success went from `0.297` with fresh-only AE to
  `0.874` with replay AE, and old cycle-3 went from `0.577` to `0.847`.
  But on fresh cycle-4 fakes the replay AE was slightly worse:
  margin success dropped from `0.842` to `0.795`.
- Interpretation:
  the principle is sound only if the AE can keep both old and fresh bad
  behavior high-energy. In this test replay protected old mistakes but diluted
  the fresh cycle-4 boundary, and controller training got worse. Replay is not
  rejected forever, but naive equal replay is not the clean fix for this
  transition.
