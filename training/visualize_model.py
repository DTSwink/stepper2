...
<body>
  ...
  <script>
    ...
    for (const item of motions) {{
      ...
      item.predTf = new Float32Array(Uint8Array.from(atob(item.pred_tf_b64), c => c.charCodeAt(0)).buffer);
      item.predTfBasis = new Float32Array(Uint8Array.from(atob(item.pred_tf_basis_b64), c => c.charCodeAt(0)).buffer);
    ...
    function draw() {{
      ...
      const pred = predictionMode === "autoregressive" ? motion.predAr : motion.predTf;
      const predBasis = predictionMode === "autoregressive" ? motion.predArBasis : motion.predTfBasis;
      const err = predictionMode === "autoregressive" ? motion.errAr : motion.errTf;
...
def rollout_model(
    ...
    gt_pos = clip.global_pos[:frame_count].cpu().numpy().astype(np.float32)
    gt_rot = clip.global_rot[:frame_count].cpu().numpy().astype(np.float32)
    pred_ar_pos = np.zeros_like(gt_pos)
    pred_tf_pos = np.zeros_like(gt_pos)
    pred_ar_rot = np.zeros_like(gt_rot)
    pred_tf_rot = np.zeros_like(gt_rot)

    pred_ar_pos[:2] = gt_pos[:2]
    pred_tf_pos[:2] = gt_pos[:2]
    pred_ar_rot[:2] = gt_rot[:2]
    pred_tf_rot[:2] = gt_rot[:2]
    ...
    for target in range(2, frame_count):
        ...
        root_pos = clip.root_pos[target_idx.cpu()].to(device)
        root_rot = clip.root_rot[target_idx.cpu()].to(device)
        ...
        tf_inp = tl.build_input(clip, tf_prev_idx, tf_cur_idx, tf_prev_pose, tf_cur_pose, cfg, device)
        tf_raw_out = tl.predict_next_raw(model, tf_inp, tf_cur_pose, cfg)
        tf_pose, _ = tl.output_to_pose(tf_raw_out, clip)
        tf_global_pos, tf_global_rot, _ = tl.fk_from_pose(clip, root_pos, root_rot, tf_pose, device)
        pred_tf_pos[target] = tf_global_pos[0].detach().cpu().numpy()
        pred_tf_rot[target] = tf_global_rot[0].detach().cpu().numpy()
    ...
    error_tf = np.linalg.norm(pred_tf_pos - gt_pos, axis=-1).mean(axis=-1).astype(np.float32)
    return gt_pos, gt_rot, pred_tf_pos, pred_tf_rot, error_tf, pred_ar_pos, pred_ar_rot, error_ar
...
def make_payload(
    ...
    gt_pos: np.ndarray,
    gt_rot: np.ndarray,
    pred_tf_pos: np.ndarray,
    pred_tf_rot: np.ndarray,
    error_tf: np.ndarray,
    pred_ar_pos: np.ndarray,
    pred_ar_rot: np.ndarray,
    error_ar: np.ndarray,
) -> dict:
    both = np.concatenate((gt_pos, pred_tf_pos, pred_ar_pos), axis=1)
    ...
    return {
        ...
        "pred_tf_b64": base64.b64encode(np.ascontiguousarray(pred_tf_pos, dtype=np.float32).tobytes()).decode("ascii"),
        "pred_tf_basis_b64": base64.b64encode(np.ascontiguousarray(pred_tf_rot, dtype=np.float32).tobytes()).decode("ascii"),
        ...
        "err_tf_b64": base64.b64encode(np.ascontiguousarray(error_tf, dtype=np.float32).tobytes()).decode("ascii"),
...
def main() -> None:
    ...
    gt_pos, gt_rot, pred_tf_pos, pred_tf_rot, error_tf, pred_ar_pos, pred_ar_rot, error_ar = rollout_model(
        model, clip, cfg, device, args.max_frames
    )
    payload = make_payload(clip, gt_pos, gt_rot, pred_tf_pos, pred_tf_rot, error_tf, pred_ar_pos, pred_ar_rot, error_ar)
    ...
    print(f"one_step_mean_joint_error_avg {float(error_tf.mean()):.6f}")
    print(f"one_step_mean_joint_error_max {float(error_tf.max()):.6f}")
