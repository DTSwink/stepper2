# IK Journal - Old Full Log

This is the archived, uncompressed working log as of 2026-05-23 before the live
`JOURNAL.md` was rewritten into a compact context-refresh handoff.
...
- TensorBoard cleanup at that point, later superseded by the stricter three-loss-card rule below:
...
## 2026-05-22 IK Foot Envelope Performance Audit

- Stopped the first corrected Walk_F fine-tune because it was silently slow:
  - it printed only at step 1 and then every 100 steps;
  - the corrected K32 envelope update was several seconds per optimizer step, so that logging cadence hid the problem.
- Root cause:
  - the envelope loss was using full-body `fk_from_pose(...)` to recover foot/toe state;
  - IK already predicts foot position/rotation and toe hinge explicitly, so full FK was unnecessary for the foot-slide envelope hot path.
- Implemented the IK-local compact path:
  - decodes only `foot_l`, `ball_l`, `foot_r`, `ball_r` from the IK vector;
  - preserves the same leg reach clamp used by full FK;
  - carries compact current foot/toe state across rollout steps instead of recomputing it;
  - computes the same linear and angular excess against the same runtime KNN envelope.
- Equivalence checks:
  - compact IK foot/toe positions vs full FK max delta about `4.8e-7 m`;
  - compact IK foot/toe rotations vs full FK max delta about `1.8e-7`;
  - envelope excess vs full FK path max delta about `1.6e-7` before vectorizing contact math;
  - vectorized contact solve keeps angular exact and changes selected linear speed by about `3.6e-4 m/s` max in the tested Walk_F batch, a numerical solve-order difference.
- Additional pure-performance changes:
  - vectorized both feet and both foot/toe sole rectangles in one compact contact solve;
  - replaced duplicate linear/angular KNN distance passes with one shared distance pass;
  - avoided decoding all core rotations just to locate leg bases.
- Benchmarks after optimization:
  - Walk_F K32 mixed, batch 119: envelope update roughly `0.94s` vs AE-only roughly `0.22s`;
  - full dataset K32 mixed, batch 1024: envelope update roughly `1.02s` vs AE-only roughly `0.22s`;
  - full dataset K32 mixed, batch 4096: envelope update roughly `1.35s` vs AE-only roughly `0.23s`.
- Tried `torch.compile` on the model/AE path:
  - large warmup;
  - slower steady-state;
  - failed with a CUDA graph pool error on repeated backward;
  - not enabled.
- Walk_F fine-tune harness changed to be honest about speed:
  - `TRAIN_STEPS = 120`;
  - logs every `10` steps;
  - saves `latest` every `30` steps.

## 2026-05-22 Animation-Dependent Envelope Correction

- Changed the envelope design again after realizing a global situation bound can mix incompatible animations:
  - the bound is now selected from the current animation plus the runtime situation;
  - example: a Walk_F rollout compares only against Walk_F situations, not turn/circle/transition rows that happen to share similar yaw/bend/foot-distance values.
- Cache version bumped to `7` after trimming unused global lookup artifacts.
- New envelope lookup:
  - exact KNN inside each row's own clip;
  - situation feature remains `[yaw_delta/pi, bend_angle/pi, runtime_horizontal_foot_distance_xz_m]`;
  - no frame index is used as a bound key.
- GT sanity after the change:
  - GT linear excess mean/p95/max: `0.0/0.0/0.0`;
  - GT angular excess mean/p95/max: `0.0/0.0/0.0`.
- Rejected performance shortcuts:
  - global grid/candidate lookup was faster but changed the actual envelope and failed the "GT below its own envelope" or "same excess as exact KNN" checks;
  - carrying contact rectangle specs through the rollout made backward slower;
  - vectorizing the foot decoder slightly changed positions and made backward slower, so the simple two-leg decoder is kept.
- Current full-dataset K32 mixed benchmark, batch `4096`:
  - AE-only update around `0.25s`;
  - animation-dependent exact envelope update around `0.9s-1.2s`;
  - latest line timing showed the actual animation KNN is no longer the bottleneck, about `0.03s`; most remaining cost is compact contact geometry, decode-next state, and backward.
- Simplified weight handling after review:
  - no envelope signature guard machinery;
  - when refinement weights are needed, they are computed from the current store's current animation-local envelope and saved;
  - a one-animation run and a full-dataset run use the same envelope code path: current clip id plus current situation.
- Containment cleanup:
  - `contact_physics.py` and `body_mass.py` now live inside `training/ik`;
  - IK imports resolve those local copies instead of the parent `training` folder.

## 2026-05-22 Training Harness Rule

- Hard rule: controller training must have one canonical training harness.
- New experiments must change dataset selection, initial checkpoint, loss weights, curriculum settings, or run labels through configuration/arguments/data selection, not by copying/rewriting a new training file.
- Do not create one-off trainer files such as a WalkF-only fine-tune script. A mini experiment must exercise the same trainer path as the full-dataset experiment so the result is trustworthy and comparable.
- If a requested experiment needs behavior that the canonical trainer cannot express, extend the canonical trainer in a general way first, then run the experiment through that path.
- Rationale: duplicated training files make it impossible to know whether a mini-run and the full run are actually training the same objective, using the same scaling, TensorBoard tags, checkpointing, crash behavior, curriculum, and envelope code.
- Follow-up fix after the Walk_F slide run stopped at step `120` while still trending down:
  - that stop came from the old one-off Walk_F file's fixed `TRAIN_STEPS = 120`, not from the stall rule;
  - the canonical full envelope trainer now accepts `--npz`, `--periodic-folder`, and `--nonperiodic-folder`, so one-animation and full-dataset controller runs use the same stall-based path;
  - future Walk_F-only controller experiments must run through `train_full_ae_envelope.py` with a dataset override, not through the old fixed-step fine-tune file;
  - the old `finetune_walkf_slide.py` training file was removed to prevent accidental reuse.
- TensorBoard controller-loss card rule:
  - only three `loss/*` curves are written during controller training: `loss/ae_score`, `loss/weighted_slide_excess`, and `loss/weighted_yaw_excess`;
  - raw physical excess values stay available through diagnostics/viewers when needed, but are not training-dashboard cards.

## 2026-05-23 Watchdog Cleanup

- Watchdog rule: restart/duplicate handling must live in one guarded script, not in ad hoc heartbeat prompts.
- `watch_supervised_run.ps1` now:
  - holds a named mutex so two wakeups cannot both decide to start training;
  - matches only Python commands running `training/ik/train.py` with the requested run label;
  - keeps the process list as an array so a single live trainer is counted correctly;
  - reports the live PID and latest checkpoint when training is already running;
  - starts from `latest`, then `last`, then the configured baseline only when `-StartIfMissing` is explicitly supplied.
- The heartbeat now calls the guarded script with `-StartIfMissing -KillDuplicates` instead of duplicating launch logic.
- Verified while the full supervised continuation was running:
  - watcher reports PID `22144`;
  - guarded start path keeps exactly one matching process;
  - no new trainer is spawned when the existing run is alive.
