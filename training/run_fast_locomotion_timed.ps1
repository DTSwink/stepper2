param(
    ...
    [string]$Prefix = ("fast_k${MaxK}_h${HiddenDim}_p${Polish}_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
)
...
try {
    ...
    $Stage2 = "${Prefix}_stage2_k$MaxK"
    $Stage3 = "${Prefix}_stage3_k${MaxK}_polish"
    $Stage4 = "${Prefix}_stage4_k${MaxK}_final"
    ...
    $Common = @(
        ...
        "--num-hidden-layers", "$NumHiddenLayers",
        "--no-compile",
        "--profile-timing",
    ...
    if ($Polish -ge 1) {
        Run-Stage "stage3_k${MaxK}_polish_5e-6" $Stage3 ($Common + @(
            "--run-name", $Stage3,
    ...
    if ($Polish -ge 2) {
        Run-Stage "stage4_k${MaxK}_final_2e-6" $Stage4 ($Common + @(
            "--run-name", $Stage4,
